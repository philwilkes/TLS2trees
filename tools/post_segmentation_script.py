    ...
    def process_point_cloud(self, point_cloud=np.zeros((0, 1))):
        if point_cloud.shape[0] == 0:
            ...
            self.point_cloud = np.array(
                pd.read_csv(self.directory + "data/segmented_point_clouds/" + self.point_cloud_filename + '.csv',
        ...
        self.point_cloud[:, self.label_index] = self.point_cloud[:,
                                                self.label_index] + 1  # index offset since noise_class was removed from inference.

        # Generate first run digital terrain model based on "terrain_points" then save DTM.csv
        self.terrain_points = self.point_cloud[self.point_cloud[:,
                                               -2] == self.terrain_class_label]  # -2 is now the class label as we added the height above DTM column.
        ...
        self.point_cloud = self.get_heights_above_DTM(
            self.point_cloud)  # Add a height above DTM column to the point clouds.
        ...
        self.terrain_points = self.terrain_points[
            np.logical_and(self.terrain_points[:, -1] > -0.1, self.terrain_points[:, -1] < 0.1)]
        # self.DTM = self.make_DTM(smoothing_radius=3*self.parameters['fine_grid_resolution'],crop_dtm=True)
        # pd.DataFrame(self.DTM).to_csv(self.output_dir+'DTM2'+'.csv',header=None,index=None,sep=' ')

        pd.DataFrame(self.terrain_points).to_csv(self.output_dir + 'terrain_points' + '.csv', header=None, index=None,
                                                 sep=' ')
        ...
        pd.DataFrame(self.stem_points).to_csv(self.output_dir + 'stem_points' + '.csv', header=None, index=None,
                                              sep=' ')
        ...
        pd.DataFrame(self.vegetation_points).to_csv(self.output_dir + 'vegetation_points' + '.csv', header=None,
                                                    index=None, sep=' ')
        ...
        try:
            self.preprocessing_total_time = np.loadtxt(
                self.parameters['directory'] + "data/postprocessed_point_clouds/" + self.parameters[
                                                                                        'input_point_cloud'][
                                                                                    :-4] + "_out/preprocessing_time.csv")
        except:
            self.preprocessing_total_time = 0
        try:
            self.sem_seg_total_time = np.loadtxt(
                self.parameters['directory'] + "data/postprocessed_point_clouds/" + self.parameters[
                                                                                        'input_point_cloud'][
        ...
        pd.DataFrame(report_array,
                     columns=['Number of Points',
                              ...
                              'Total run time (s)']).to_csv(self.output_dir + 'post_segmentation_report' + '.csv',
