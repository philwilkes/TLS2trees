...
class ReportWriter:
    def __init__(self, parameters):
        ...
        # self.ground_veg[:, self.veg_dict['z']] = 0
        # self.ground_veg = subsample_point_cloud(self.ground_veg, 0.01)
    ...
    def create_report(self):
        ...
        mdFile.new_header(level=level, title='Plot Radius: ' + str(self.parameters['plot_radius']) + ' m, ' + ' Plot Radius Buffer: ' + str(self.parameters['plot_radius_buffer']) + ' m, Plot Area: '+ str(plot_area) + ' ha')
        mdFile.new_header(level=level, title='Stems/ha:  ')
        mdFile.new_header(level=level, title='Mean DBH:')
        mdFile.new_header(level=level, title='Median DBH:')
        mdFile.new_header(level=level, title='Min DBH:')
        mdFile.new_header(level=level, title='Max DBH:')
    ...
    def plot_outputs(self):
        treeNo = np.array(self.tree_data['treeNo'])
        x_tree_base = np.array(self.tree_data['x_tree_base'])
        y_tree_base = np.array(self.tree_data['y_tree_base'])
        DBH = np.array(self.tree_data['DBH'])
        Height = np.array(self.tree_data['Height'])
        Volume = np.array(self.tree_data['Volume'])
        ...
        ax5.set_xlabel("Easting - Local (m)")
        ax5.set_ylabel("Northing - Local (m)")
        ...
        ax5.fill(self.DTM[vertices, 0] - plot_centre[0], self.DTM[vertices, 1] - plot_centre[1], c='white', alpha=1, zorder=0)
        ax5.plot(self.DTM[vertices, 0] - plot_centre[0], self.DTM[vertices, 1] - plot_centre[1], c='k', alpha=1, linewidth=0.5, zorder=1)
        ...
        ax5.scatter(x_tree_base - plot_centre[0], y_tree_base - plot_centre[1], marker='.', s=70, c='black', zorder=7)
        ax5.scatter(x_tree_base - plot_centre[0], y_tree_base - plot_centre[1], marker='.', s=30, c='red', zorder=8)

        tree_label_offset = np.array([-0.01, 0.01]) * plot_max_distance
        for i in range(0, x_tree_base.shape[0]):
            ax5.text((x_tree_base[i] - plot_centre[0]) + tree_label_offset[0], (y_tree_base[i] - plot_centre[1]) + tree_label_offset[1], treeNo[i], fontsize=6, zorder=9)
