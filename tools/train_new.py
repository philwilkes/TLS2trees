...
class TrainingDataset(Dataset):
    ...
    def augmentations(self,X,no_terrain,no_cwd):
        ...
        if no_terrain and no_cwd:
            rotations = [np.random.uniform(-90,90),np.random.uniform(-90,90),np.random.uniform(-180,180)]#np.random.randint(0,3)*90]
        else:
            rotations = [np.random.uniform(-5,5),np.random.uniform(-5,5),np.random.uniform(-180,180)]#np.random.randint(0,3)*90]
...
if __name__ == '__main__':
    ...
    def folder_structure(parameters):
        ...
        if not os.path.isdir(parameters['directory']+"data/working_directory/"+parameters['input_point_cloud'][:-4]):
    ...
    if 0:
        ...
        preprocessing.load_point_cloud(filename=parameters['directory']+'data/original_point_clouds/'+parameters['input_point_cloud'])
    ...
    train_dataset = TrainingDataset(dataset_name = 'train_aug',
                                    root_dir = parameters['directory']+"data/working_directory/"+parameters['input_point_cloud'][:-4]+'/',
    ...
    if parameters['load_existing_model']:
        ...
        model.load_state_dict(torch.load(parameters['directory']+'model/'+parameters['model_filename']),strict=False)
    ...
    if parameters['load_existing_model']:
        try:
            history_array = np.array(pd.read_csv(parameters['directory']+'model/'+'history_array' +parameters['model_filename'][:-4]+'.csv',header=None,index_col=None,delim_whitespace=True))
        ...
        for i, data in enumerate(train_loader):
            ...
            if (i + 1) % 500 == 0:
                ...
                for j, data in enumerate(test_loader):
                    ...
                    if (j+1) % 10 == 0:
                        visualiser = np.vstack((visualiser,np.hstack((data.pos.cpu()+np.array([j,0,0]),np.atleast_2d(out).T,np.atleast_2d(data.y.cpu()).T))))
                        
                    if (j + 1) % 100 == 0:
                        pd.DataFrame(visualiser).to_csv(parameters['directory']+'most_recent_out.csv',header=None,index=None,sep=' ')
                        break
                ...
                pd.DataFrame(history_array).to_csv(parameters['directory']+'model/'+'history_array' +parameters['model_filename']+'.csv',header=None, index=None,sep=' ')
                model.train()
                samples_seen = i + samples_seen
        num_epochs_seen += 1
        torch.save(model.state_dict(),parameters['directory']+'model/'+parameters['model_filename'][:-4]+'_'+str(num_epochs_seen)+'.pth')
        # pd.DataFrame(history_array).to_csv(parameters['directory']+'model/'+'history_array' +parameters['model_filename'][:-4]+'.csv',header=None, index=None,sep=' ')
        # scheduler.step()
